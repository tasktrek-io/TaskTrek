  ...
  const loadWorkspaceData = async () => {
    ...
    try {
      ...
      try {
        ...
        const workspaceProjects = allProjectsResponse.data.filter(
          (p: Project) => p.workspace === currentWorkspace._id
        );
  ...
  return (
    <>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-8'>
        {/* Total Projects */}
        <div
          className='bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md dark:hover:shadow-lg dark:hover:shadow-gray-900/20 transition-shadow cursor-pointer'
        ...
        <div className='bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md dark:hover:shadow-lg dark:hover:shadow-gray-900/20 transition-shadow'>
          ...
          <div className='text-xs text-gray-500 dark:text-gray-400'>
            {assignedTasks.filter(t => t.priority === 'urgent' && t.status === 'todo').length}{' '}
            urgent tasks
          </div>
        </div>
        ...
        <div className='bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md dark:hover:shadow-lg dark:hover:shadow-gray-900/20 transition-shadow'>
          ...
          <div className='text-xs text-gray-500 dark:text-gray-400'>
            {Math.round((inProgressTasks.length / Math.max(assignedTasks.length, 1)) * 100)}% of
            total tasks
          </div>
        </div>
      </div>
              ...
              <>
                ...
                <div className='mt-4 pt-3 border-t border-gray-100 dark:border-gray-700'>
                  <div className='flex justify-between text-xs text-gray-500 dark:text-gray-400'>
                    ...
                    <span>
                      Most: {Math.max(urgentTasks, highTasks, mediumTasks, lowTasks)}{' '}
                      {urgentTasks === Math.max(urgentTasks, highTasks, mediumTasks, lowTasks)
                        ? 'urgent'
                        : highTasks === Math.max(urgentTasks, highTasks, mediumTasks, lowTasks)
                          ? 'high'
                          : mediumTasks === Math.max(urgentTasks, highTasks, mediumTasks, lowTasks)
                            ? 'medium'
                            : 'low'}
                    </span>
                  </div>
            ...
            {(() => {
              ...
              return upcomingTasks.map(task => {
                ...
                return (
                  <div
                    key={task._id}
                    className='flex items-start gap-3 p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors'
                  >
                    <div
