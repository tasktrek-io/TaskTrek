...
export default function MyTasksPage() {
  ...
  const filterTasksByWorkspace = async () => {
    ...
    try {
      ...
      const filteredTasks = allTasks.filter(task => workspaceProjectIds.includes(task.project._id));
  ...
  const updateTaskStatus = async (taskId: string, status: Task['status']) => {
    try {
      ...
      setAllTasks(prev => prev.map(task => (task._id === taskId ? { ...task, status } : task)));
  ...
  const filteredTasks =
    filter === 'all' ? workspaceTasks : workspaceTasks.filter(task => task.status === filter);
