...
export default function ProjectPage() {
  ...
  const { isUserOnline, joinProject, leaveProject, joinOrganization, leaveOrganization } =
    useSocket();
  ...
  // Comment edit and delete states
  const [editingComment, setEditingComment] = useState<string | null>(null);
  const [editCommentContent, setEditCommentContent] = useState('');
  const [showCommentMenu, setShowCommentMenu] = useState<{ [commentId: string]: boolean }>({});
  ...
  const addReaction = async (commentId: string, emoji: string) => {
    ...
    try {
      const response = await api.post(
        `/tasks/${selectedTask._id}/comments/${commentId}/reactions`,
        {
          emoji,
        }
      );
      ...
      setComments(prev =>
        prev.map(comment => (comment._id === commentId ? response.data : comment))
      );
  ...
  const saveCommentEdit = async () => {
    if (!selectedTask || !editingComment || !editCommentContent.trim()) return;

    try {
      const response = await api.patch(`/tasks/${selectedTask._id}/comments/${editingComment}`, {
        content: editCommentContent.trim(),
      });

      // Update the comment in the comments list
      setComments(prev =>
        prev.map(comment => (comment._id === editingComment ? response.data : comment))
      );
  ...
  const deleteComment = async (commentId: string) => {
    ...
    try {
      await api.delete(`/tasks/${selectedTask._id}/comments/${commentId}`);
  ...
  const handleCommentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (mentionMatch) {
      ...
      const suggestions = allMembers.filter(
        member =>
  ...
  const addMember = async (memberId: string) => {
    ...
    if (project) {
      const isAlreadyMember =
        project.owner._id === memberId || project.members.some(member => member._id === memberId);
  ...
  const toggleTaskFilter = (
    filterType: 'status' | 'priority' | 'assignees' | 'tags',
    value: string
  ) => {
    setTaskFilters(prev => ({
      ...prev,
  ...
  const filterAndSortTasks = (tasks: Task[]) => {
    ...
    if (taskFilters.status.length > 0) {
      filteredTasks = filteredTasks.filter(task => taskFilters.status.includes(task.status));
    }

    // Apply priority filter
    if (taskFilters.priority.length > 0) {
      filteredTasks = filteredTasks.filter(task => taskFilters.priority.includes(task.priority));
    }
      ...
      filteredTasks = filteredTasks.filter(task => {
        if (taskFilters.assignees.includes('unassigned')) {
          return (
            ...
            task.assignees.some(assignee => taskFilters.assignees.includes(assignee._id))
          );
        }
        return task.assignees.some(assignee => taskFilters.assignees.includes(assignee._id));
      });
    }
    ...
    if (taskFilters.dueDateRange.start || taskFilters.dueDateRange.end) {
      filteredTasks = filteredTasks.filter(task => {
        ...
        const startDate = taskFilters.dueDateRange.start
          ? new Date(taskFilters.dueDateRange.start)
          : null;
        const endDate = taskFilters.dueDateRange.end
          ? new Date(taskFilters.dueDateRange.end)
          : null;
      ...
      if (query && project?.workspace) {
        api
          ...
          .then(r => {
            ...
            const filteredResults = r.data.filter(
              (user: Member) => !currentMemberIds.has(user._id)
            );
  ...
  return (
    <>
      ...
      <div className='bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6'>
        <div className='flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4'>
          <div className='flex-1 min-w-0'>
            <h1 className='text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2 truncate'>
              ...
              {project?.tags && project.tags.length > 0 && (
                <div className='flex flex-wrap gap-1'>
                  {project.tags.map(tag => (
                    <span
                      key={tag}
                      className='px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded text-xs whitespace-nowrap'
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <div className='flex gap-3 items-center flex-shrink-0'>
            ...
            {project &&
              currentUser &&
              (project.owner._id === currentUser._id || canUserDeleteProject) && (
      ...
      <section className='mt-8 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-6'>
        <div className='font-medium mb-4 text-sm sm:text-base text-gray-900 dark:text-gray-100'>
          Team Members
        </div>
          ...
          {allMembers.map(m => (
            <div key={m._id} className='relative'>
              <span className='px-2 sm:px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-xs sm:text-sm truncate max-w-full inline-flex items-center gap-2'>
                ...
                {m.name} ({m.email}){/* Online status text indicator */}
                {isUserOnline(m._id) && (
          ...
          <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
            ...
            <div className='flex flex-col sm:flex-row items-stretch sm:items-center gap-2 overflow-x-auto'>
              ...
              <div className='relative min-w-0 flex-shrink-0'>
                ...
                />
                <svg
                  className='w-4 h-4 absolute left-2.5 top-3 text-gray-400 dark:text-gray-500'
                  fill='none'
                  stroke='currentColor'
                  viewBox='0 0 24 24'
                >
                        ...
                        <div
                          ...
                          className={`space-y-4 min-h-[200px] p-3 rounded-lg transition-colors ${
                            snapshot.isDraggingOver
                              ? 'bg-blue-50 dark:bg-blue-900 border-2 border-blue-200 dark:border-blue-600 border-dashed'
                          ...
                          {statusTasks.map((task, index) => (
                            <Draggable key={task._id} draggableId={task._id} index={index}>
                              {(provided, snapshot) => (
                                ...
                                >
                                  <h4 className='font-medium text-gray-900 dark:text-gray-100 mb-3 text-sm sm:text-base'>
                                    {task.title}
                                  </h4>

                                  <div className='flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-4'>
                                    <span
                                      className={`px-2 py-1 rounded text-xs font-medium w-fit ${getPriorityColor(task.priority)}`}
                                    >
                                      {task.priority}
                                    </span>
                                    {task.dueDate && (
          ...
          <div className='space-y-2'>
            ...
            {Object.entries(tasksByStatus).map(([status, statusTasks]) => {
              ...
              return (
                <div
                  key={status}
                  className='bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg overflow-hidden'
                >
                  {/* Section Header */}
                  <div
                ...
                <div className='lg:col-span-2 space-y-6'>
                  ...
                  <div className='space-y-4 size-y-6'>
                    <div className='border border-gray-200 dark:border-gray-600 rounded-lg'>
                      <TaskDocuments taskId={selectedTask._id} canEdit={true} className='' />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className='space-y-4'>
                      {comments.map(comment => (
                        <div
                          key={comment._id}
                          className='border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg p-4'
                        >
                          <div className='flex items-center justify-between mb-2'>
                            ...
                            {/* Comment Menu - Only show for comment author */}
                                ...
                                {showCommentMenu[comment._id] && (
                                  <div className='absolute right-0 top-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-10 min-w-[120px]'>
                                    ...
                                    <button
                                      onClick={() => {
                                        setShowCommentMenu({});
                                        setShowCommentDeleteConfirm(comment._id);
                                      }}
                          ...
                          {editingComment === comment._id ? (
                            <div className='space-y-3'>
                              <textarea
                                ...
                                className='w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none'
                                rows={3}
                              />
                              <div className='flex gap-2'>
                                <button
                                ...
                                <button
                                  ...
                                  className='px-3 py-1 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 text-sm rounded hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors'
                                >
                                  Cancel
                                </button>
                              </div>
                            ...
                            <>
                              ...
                              <div className='flex flex-wrap gap-2 items-center'>
                                ...
                                {comment.reactions
                                  ?.filter(r => r.count > 0)
                                  .map((reaction, index) => {
                                    // Get the names of users who reacted with this emoji
                                    const reactorNames =
                                      reaction.users
                                        ?.map((user: any) => {
                                          ...
                                          if (typeof user === 'object' && user.name) {
                                            ...
                                            // Fallback to finding user in allMembers if it's just an ID
                                            const member = allMembers.find(
                                              member => member._id === user || member.id === user
                                            );
                                    ...
                                    const tooltipText =
                                      reactorNames.length > 0
                                        ? `${reactorNames.join(', ')} reacted with ${reaction.emoji}`
                                        : `${reaction.count} reaction${reaction.count > 1 ? 's' : ''} with ${reaction.emoji}`;

                                    return (
                                      <button
                                        key={`${reaction.emoji}-${index}`}
                                        onClick={() => addReaction(comment._id, reaction.emoji)}
                                        className='flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-gray-600 hover:bg-gray-200 dark:hover:bg-gray-500 rounded-full text-sm transition-colors group relative'
                                        title={tooltipText}
                                      >
                  ...
                  {/* Activity Section */}
                  <TaskActivity taskId={selectedTask._id} onActivitiesLoad={handleActivitiesLoad} />
                </div>
                            ...
                            {assigneeSearchResults.map(user => (
                              ...
                              >
                                ...
                                {selectedTask.assignees.some(a => a._id === user._id) && (
                                  <span className='ml-auto text-xs text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900 px-2 py-1 rounded'>
                                    Already assigned
                                  </span>
                                )}
                              </button>
                        ...
                        <div className='flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg'>
                          <div className='flex items-center gap-2 flex-1'>
                            ...
                            <div>
                              ...
                              <p className='text-xs text-gray-600 dark:text-gray-400'>
                                Get notified of all activity on this task.
                              </p>
                            </div>
                          </div>
                          <button
              ...
              <form onSubmit={updateProject} className='space-y-4'>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={e =>
                      setProjectEditForm(prev => ({ ...prev, description: e.target.value }))
                    }
                    className='w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500'
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                  <div>
                    ...
                    <select
                      ...
                      onChange={e =>
                        setProjectEditForm(prev => ({ ...prev, status: e.target.value as any }))
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setProjectEditForm(prev => ({ ...prev, startDate: e.target.value }))
                      }
                      className='w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500'
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setProjectEditForm(prev => ({ ...prev, endDate: e.target.value }))
                      }
                      className='w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500'
                    />
                  </div>
            ...
            <div className='p-6'>
              ...
              <div className='mb-6'>
                <p className='text-gray-700 dark:text-gray-300'>
                  ...
                  <span className='font-semibold'>"{selectedTask.title}"</span>? This will
                  permanently delete the task, all its comments, and activity history.
                </p>
              </div>
          ...
          <div className='bg-white dark:bg-gray-800 rounded-lg max-w-md w-full'>
            <div className='p-6'>
              ...
              <div className='mb-6'>
                <p className='text-gray-700 dark:text-gray-300'>
                  Are you sure you want to delete{' '}
                  <span className='font-semibold'>"{project.name}"</span>? This will permanently
                  delete the project, all its tasks, comments, and activity history.
                </p>
              </div>
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50'>
          <div className='bg-white dark:bg-gray-800 rounded-lg max-w-md w-full'>
            <div className='p-6'>
              ...
              <div className='mb-6'>
                <p className='text-gray-700 dark:text-gray-300'>
                  Are you sure you want to delete this comment? This will permanently remove the
                  comment and its reactions.
                </p>
              </div>
              ...
              <div className='flex gap-3'>
                ...
                <button
                  type='button'
                  onClick={() => deleteComment(showCommentDeleteConfirm)}
                  disabled={isDeletingComment}
                  className='flex-1 bg-red-600 dark:bg-red-600 text-white py-2 px-4 rounded-lg hover:bg-red-700 dark:hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2'
                >
                  {isDeletingComment ? (
